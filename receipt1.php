...
<?php
	...
	$sql2 = "SELECT * FROM tblorder WHERE order_id = $oid";
...
<table border="0" width="auto">
	...
	<td width="25%"><strong>Invoice No</strong></td>
		<td width="3%"><b>:</b></td>
		<td width="auto"><?php echo$trans; ?></td>
	</tr>
	<tr>
...
<?php
	...
	$sql = "SELECT COUNT(*) FROM tblorder, tblitem, tblusers WHERE tblorder.c_id = $userid AND tblusers.userid = $userid  AND tblitem.itemid = tblorder.i_id AND tblorder.order_id = '$oid'";
	//echo$sql;
	$query = (mysqli_query($conn,$sql));
	...
	$selectproducts = "SELECT * from tblorder,tblitem, tblusers WHERE tblorder.c_id = '$userid' AND tblusers.userid = $userid AND tblitem.itemid = tblorder.i_id AND tblorder.order_id='$oid'";
	//echo$selectproducts;
	$query = mysqli_query($conn, $selectproducts);
	$rowq=mysqli_num_rows($query);
	//echo$rowq;
	$list = "";
	$src = "uploads/";
	...
	else
	{
		...
		for($loop = 0; $loop < $countrows; $loop++)
		{
			...
			while($row = mysqli_fetch_array($query, MYSQLI_ASSOC))
			{
				...
				$list ="<tr align='center'><td>" . $itemid . "</td><td>" . $prodname . "</td><td>" . $qty . "</td>";
